...
void main( int argc, char **argv ) {
    ...
    double packages_before[2];
    double packages_after[2];
    double pp0_before,pp0_after;
    double pp1_before=0.0,pp1_after;
    double dram_before[2];
    double dram_after[2];
    ...
    long  double nowtime, period, power1, power2;
    FILE *file;
    ...
    // char filename[50] = "/home/cc/penelope/tools/RAPL/PowerResults.txt";
    char filename[1000];
    if (argc==2){
        strncpy(filename, argv[1], 1000);
    }
    else{
        strncpy(filename, "/home/cc/RAPL/PowerResults.txt", 1000);
    }
    ...
    result1=read_msr(fd[0],MSR_RAPL_POWER_UNIT);
    ...
    uint64_t currentval;
    currentval=read_msr(fd[0],MSR_PKG_RAPL_POWER_LIMIT);  
    printf("RAPL power limit = %.6fW\n", power_units*(double)(currentval&0x7fff));
    currentval=read_msr(fd[0],MSR_DRAM_POWER_LIMIT );  
    printf("DRAM power limit = %.6fW\n", power_units*(double)(currentval&0x7fff));
    ...
    while (1){
        ...
        for (int i = 0; i < 2; ++i){
            results[i]=read_msr(fd[i],MSR_PKG_ENERGY_STATUS); 
            packages_before[i]=(double)results[i]*energy_units;
            results[i]=read_msr(fd[i],MSR_DRAM_ENERGY_STATUS); 
            dram_before[i]=(double)results[i]*energy_units;
        }
        ...
        for (int i = 0; i < 2; ++i){
            results[i]=read_msr(fd[i],MSR_PKG_ENERGY_STATUS); 
            packages_after[i]=(double)results[i]*energy_units;
            results[i]=read_msr(fd[i],MSR_DRAM_ENERGY_STATUS); 
            dram_after[i]=(double)results[i]*energy_units;
        }
        ...
        nowtime =((long) ((currentime2.tv_usec) + (currentime2.tv_sec)* 1000000))/1000000.000000;
        period = ((currentime2.tv_usec - currentime1.tv_usec) + (currentime2.tv_sec - currentime1.tv_sec)*1000000)/1000000;
        ...
        // for (int i = 0; i < 2; ++i){
        //     if (i%2==0){
        //         power1+=packages_after[i]-packages_before[i];
        //     }
        //     else{
        //         power2+=packages_after[i]-packages_before[i];
        //     }
        // }
        // power1/=period;
        // power2/=period;
        ...
        fprintf(file,"%LF %LF %LF %LF %LF\n", nowtime, (packages_after[0]-packages_before[0])/period, 
            (packages_after[1]-packages_before[1])/period, dram_after[0]-dram_before[0]/period,
            dram_after[1]-dram_before[1]/period);

        // fprintf(file, "%LF", nowtime);
        // for (int i = 0; i < 2; ++i){
        //     long double power=(packages_after[i]-packages_before[i])/period;
        //     fprintf(file, " %LF", power);
        // }
        // fprintf(file, "\n");
